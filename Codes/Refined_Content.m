clear Refined_data Truth_value Refined_data_FinalV R pca_data
%%
...
for idt = 2:size(Refined_data_FinalV, 2)
    R{idt} = corrcoef(Truth_value,cell2mat(Refined_data_FinalV(:,idt)));
    if (R{idt}(1,2) <= -0.1 || R{idt}(1,2) >= 0.1)
        pca_data(:,idt-1) = Refined_data(:,idt); % idt-1 eliminates the epoch
    end
end
