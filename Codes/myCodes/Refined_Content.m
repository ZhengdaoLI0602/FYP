...
for idt = 1: size(GNSS_data,1)  %number of epoches
    ...
    Refined_data {idt,2} = sqrt(GNSS_data{idt,16}(3)^2); % EPV
    Refined_data {idt,3} = sqrt(GNSS_data{idt,16}(1)^2  +...
                                                    GNSS_data{idt,16}(2)^2); % EPH
    Refined_data {idt,4} = sqrt(GNSS_data{idt,16}(1)^2  +...
                                                    GNSS_data{idt,16}(2)^2 + GNSS_data{idt,16}(3)^2); % positioning error
    Refined_data {idt,5} = length(GNSS_data{idt,3});  % number of SV ##
    Refined_data {idt,6} = mean(GNSS_data{idt,4}); % mean elevation angle
    Refined_data {idt,7} = var(GNSS_data{idt,4}); % variance elevation angle
    
    Refined_data {idt,8} = mean(GNSS_data{idt,9}(:,4),'all'); % C/N0
    
    Refined_data {idt,9} = mean(GNSS_data{idt,9}(:,9),'all'); % Pr residual RSS_e
    Refined_data {idt,10} = mean(GNSS_data{idt,9}(:,6),'all'); % mean Pr residual
...
%% Final Refinement
%Truth_value(:,1) = sqrt(cell2mat(Refined_data (:,2)).^2 + cell2mat(Refined_data (:,3)).^2 ); % truth value for positioing error
